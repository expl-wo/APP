  ...
  <view class="production-detail-root">
    ...
    <view class="production-content">
      <view class="production-info">
        <view class="production-name title">
          <view>{{ productionName }}</view>
          <view class="operate-icon">
            <text class="tip" @click="showTipModal">工序要求</text>
            <image
              src="@/assets/imgs/staging/add-issue-icon.png"
              alt=""
              @click="handleAddIssue"
            />
          </view>
        </view>
        <Info :fieldMapText="fieldMapText" :infoObj="productionDetailInfo" />
        <van-tabs
          v-model="activeTab"
          line-width="12"
          line-height="4"
